...
//**LATER DELETE LINK we cannot use in shared shared library in upper folder
const FreeformShared    = require('./../FreeformShared').FreeformShared;
const GlobalStorage     = require('./../GlobalStorage/GlobalStorage').GlobalStorage;

class AccessListShared {// FreeformAccessListForModel
    constructor() {
    ...
    static async checkListForAccessToFreeformObject (iNoperation, iNfolder,iNobjModelId, iNobjInId, iNuser, iNmodelId, iNformId, InExistModel, iNclientData = null) { //+
        ...
        for (let accessTypeKey of Object.keys(accessType) ){
            ...
            try {
                let checkAccessOperation = await AccessListShared.checkAccessOperationToFreeformObject (
                    iNoperation,
                    iNfolder,
                    objModelId,
                    objInId,
    ...
    static async checkAccessOperationToFreeformObject (iNoperation, iNfolder, iNobjModelId, iNobjInId, iNuser, iNmodelId, iNformId, InExistModel, iNclientData, iNaccessType) { //-
        // private
        // invoke - ONLY after checked model isset && check user token
        ...
        switch (accessType) {
            ...
            case '@list':
                ...
                let accessObject;
                try {
                    accessObject = await AccessListShared.getAccessObjectFromFormForUser (user, modelId, formId, listId, clientData['uid']);
                }catch (e) {
                ...
                try {
                    hasAccessForCreateObject = AccessListShared.checkAccessForObject (folder, operation, accessObject, objModelId, objInId );
    ...
    static checkAccessForObject (iNfolder, iNoperation, iNaccessData, iNobjModelId = null, iNobjInId = null ) { //-
        //@private
        ...
        if (
            ...
            let accessOperation = AccessListShared.checkAccessForOperation (operation, accessData['form'] , null, null );
            ...
            if (
                ...
                ( accessOperation === null && AccessListShared.checkAccessForOperation (operation, accessData['*'] ,null, null ) )
            ) {
            ...
            let accessOperation = AccessListShared.checkAccessForOperation ( operation, accessData[folder] , objModelId, objInId );
            ...
            if (
                ...
                ( accessOperation === null && AccessListShared.checkAccessForOperation (operation, accessData['form'] , null, null ) )
            ) {
    ...
    static checkAccessOperation (iNobj, iNoperation) {
        if (
            iNobj[iNoperation]
            ...
            return AccessListShared.checkAccessOperation ( iNobj, '*' );
    ...
    static checkAccessForOperation (iNoperation, iNaccessObject, iNobjModelId = null, iNobjInId = null ) { //-
        //@private
        ...
        if (
            ...
            if (
                ...
                if (!(iNaccessObject[objModelId] && iNaccessObject[objModelId]['@model'])) {
                    if ( objModelId !== '*' ) {
                        let r =  AccessListShared.checkAccessForOperation(iNoperation, iNaccessObject, '*', null);
                        return r;
                    }
                } else {
                ...
                if (!iNaccessObject[objModelId]) {
                    ...
                    if (objModelId !== '*') {
                        let r = AccessListShared.checkAccessForOperation(iNoperation, iNaccessObject, '*', '*');
                    ...
                    if (objInId !== '*') {
                        let r = AccessListShared.checkAccessForOperation(iNoperation, iNaccessObject, objModelId, '*');
            ...
            if (objWithOperation) {
                ...
                let result =  AccessListShared.checkAccessOperation(objWithOperation, iNoperation);
                if (result) {
    ...
    static async  getAccessObjectFromFormForUser (iNuserLogin, iNmodelId, iNformId, iNlistId, iNuid) { //-
        //@private
        /*
          @inputs
            @required
...
//     }
    ...
    static async startCopyAccessListToFormObject (iNuser, iNformModelId, iNformObjectId, iNfirestoreBatch) { //-
        //@private
        ...
        if (typeof localAccessLists === 'object'){
            ...
            for (let accessListId of Object.keys(localAccessLists)) {
                ...
                for (let userId of Object.keys(accessList)){
                    ...
                    let writeOperation = AccessListShared.createFormObjectAccessLists(iNuser, iNformModelId, iNformObjectId, accessListId, userId, userAccessListObject, iNfirestoreBatch);
    ...
    static async createFormObjectAccessLists (iNuser, iNformModelId, iNformObjectId, iNaccessListId, iNuserId, iNdata, iNfirestoreBatchGroup) { //-
        //@private
    ...
    static async preloadAccessObjectsFromFormForUserByList (iNuserLogin, iNmodelId, iNformId, iNpreloadList, iNuid) { //+ (2)
        //@public
        if(
            !(
        ...
        for (let accessListId of preloadAccessLists ) {
            promiseArray.push(
                AccessListShared.getAccessObjectFromFormForUser (iNuserLogin, iNmodelId, iNformId, accessListId, iNuid)
            )
        }
        ...
        static async  clearFormModelFromNotAccessSubModels (iNformUser, iNmodelId, iNformId, iNclientData, iNform, iNoperation, iNfolder = null) { //+
            ...
            for (let folderName of Object.keys(folderCounter)) {
                let folder = form[folderName],
                    ...
                    result = folderCounter[folderName] = await AccessListShared.clearFolderFromNotAccessSubModels (iNformUser, iNmodelId, iNformId, iNclientData,folder, iNoperation, objectType);
        ...
        static async  clearFolderFromNotAccessSubModels (iNformUser, iNmodelId, iNformId, iNclientData,  iNfolderObject, iNoperation, iNobjectType) { //-
            ...
            for (let modelId of Object.keys(folderObj)) {
                let model   = folderObj[modelId],
                    access  = await AccessListShared.checkListForAccessToFreeformObject (
                        iNoperation,
                        iNobjectType,
                        null,
                        null,
...
module.exports.AccessListShared = AccessListShared;
