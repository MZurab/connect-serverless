    ...
    if (
      ...
      if (apiType == 'createPrivateChat') {
        ...
        var functionGetUserByLogin = (errUser,dataUser) => {
          ...
          // creating function for add main chat
          var functAddMainChat = (iNusersForFirst) => {
            CHAT.addMainChat (
              { 'with': activeUser,      'users': iNusersForFirst  },
              ...
              (errMainChat,dataMainChat) => {
                if (errMainChat) {
                  ...
                  var errorObject   = LOG.defaultErrorMessage(1, 7,'We cannot create main chat' , errMainChat, dataMainChat);
                  result['status']  = 0;
                  result['error']   = errorObject;
                  context.succeed(result);
                  return;
                ...
                LOG.step(1, 7,'We createed main chat -> create main chat with rule from trigger',errMainChat,dataMainChat);
          ...
          CHAT.getChiefChatIdByUid (
            ...
            function (chatData) {
              if ( !chatData ) {
                //ERROR (1.6) This chat is not exist => create chat
                var errorObject   = LOG.defaultErrorMessage(1,6,'This chat is not exist => create chat' , chatData);

                functAddMainChat ( [thisUserId] );
                return;
              }
